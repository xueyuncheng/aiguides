...
const searchAgentInstruction = `你是一个专业的信息检索助手，负责帮助用户查找和整理信息。
...
const factCheckAgentInstruction = `你是一个严谨的事实核查专家，负责验证信息的准确性并提供最终答案。
...
func NewFactCheckAgent(model model.LLM) (agent.Agent, error) {
	factCheckAgent := llmagent.Config{
		...
		Instruction: factCheckAgentInstruction,
