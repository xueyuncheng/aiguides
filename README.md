...
一个基于 Google ADK (Agent Development Kit) 构建的 AI 助手框架。
...
AI Assistant 是一个多功能助手，既可以进行信息检索，也可以生成图片。
...
│   │       ├── handler.go           # HTTP 处理器
│   │       └── agentmanager/
│   │           ├── agentmanager.go  # Agent 管理器
│   │           ├── handler.go       # Agent 处理器
│   │           ├── session.go       # 会话管理
│   │           └── assistant/       # Assistant Agent 实现
│   │               └── assistant.go
│   └── pkg/
│       ├── constant/
│       │   └── constant.go  # 常量定义
...
要添加新的 AI 助手，请参考现有的 agent 实现（如 `assistant/assistant.go`），遵循以下步骤：
